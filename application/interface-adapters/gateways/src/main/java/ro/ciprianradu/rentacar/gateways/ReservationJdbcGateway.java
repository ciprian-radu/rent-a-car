...
public class ReservationJdbcGateway implements ReservationGateway {
    ...
    /**
     * Saves a reservation. Note that Renter, Vehicle pickup and return location must already exist
     * in the database.
     *
    ...
    public Reservation save(final Reservation reservation) {
        jdbcTemplate.update(
            ...
            reservation.getRenter().getEmail(), reservation.getVehicle().getId(), Timestamp
                .from(reservation.getPickupDate().withZoneSameLocal(ZoneOffset.UTC).toInstant()),
            reservation.getPickupLocation().getName(), Timestamp
                .from(reservation.getReturnDate().withZoneSameLocal(ZoneOffset.UTC).toInstant()),
            reservation.getReturnLocation().getName());
        return reservation;
    }
    ...
    public Iterable<Reservation> findAllByVehicle(final String vehicleId) {
        return jdbcTemplate.query(
            "SELECT RT.FIRST_NAME AS RENTER_FIRST_NAME, RT.LAST_NAME AS RENTER_LAST_NAME, RT.EMAIL AS RENTER_EMAIL, RT.TELEPHONE_NUMBER AS RENTER_PHONE, "
                ...
                + "WHERE R.VEHICLE = ?", mapper, vehicleId);
    }
    ...
    public Optional<Reservation> findByOverlappingRentPeriod(final String vehicleId,
        ...
        final List<Reservation> reservations = jdbcTemplate.query(
            "SELECT RT.FIRST_NAME AS RENTER_FIRST_NAME, RT.LAST_NAME AS RENTER_LAST_NAME, RT.EMAIL AS RENTER_EMAIL, RT.TELEPHONE_NUMBER AS RENTER_PHONE, "
                + "V.ID AS VEHICLE_ID, VT.TYPE AS VEHICLE_TYPE, VB.NAME AS VEHICLE_BRAND, VM.NAME AS VEHICLE_MODEL, VM.AC AS VEHICLE_AC, VM.AUTOMATIC_TRANSMISSION AS VEHICLE_AUTOMATIC_TRANSMISSION, VM.SEATS AS VEHICLE_SEATS, V.RATE AS VEHICLE_RATE, "
                + "R.PICKUP_DATE, PL.NAME AS PICKUP_LOCATION_NAME, PL.ADDRESS AS PICKUP_LOCATION_ADDRESS, "
                + "R.RETURN_DATE, RL.NAME AS RETURN_LOCATION_NAME, RL.ADDRESS AS RETURN_LOCATION_ADDRESS "
                ...
                + "JOIN RENT_A_CAR.LOCATION PL ON R.PICKUP_LOCATION = PL.NAME "
                + "JOIN RENT_A_CAR.LOCATION RL ON R.RETURN_LOCATION = RL.NAME "
                + "WHERE R.VEHICLE = ? AND ((R.PICKUP_DATE >= ? AND R.PICKUP_DATE <= ?) OR (R.RETURN_DATE >= ? AND R.RETURN_DATE <= ?))",
            mapper, vehicleId,
            Timestamp.from(pickupDate.withZoneSameLocal(ZoneOffset.UTC).toInstant()),
            Timestamp.from(returnDate.withZoneSameLocal(ZoneOffset.UTC).toInstant()),
            Timestamp.from(pickupDate.withZoneSameLocal(ZoneOffset.UTC).toInstant()),
            Timestamp.from(returnDate.withZoneSameLocal(ZoneOffset.UTC).toInstant()));
    ...
    private static class ReservationRowMapper implements RowMapper<Reservation> {
        ...
        public Reservation mapRow(final ResultSet rs, final int rowNum) throws SQLException {
            ...
            mapVehicle(rs, reservation);
            reservation
                .setPickupDate(rs.getTimestamp("PICKUP_DATE").toInstant().atZone(ZoneOffset.UTC));
            reservation.setPickupLocation(mapPickupLocation(rs));
            reservation
                .setReturnDate(rs.getTimestamp("RETURN_DATE").toInstant().atZone(ZoneOffset.UTC));
            reservation.setReturnLocation(mapReturnLocation(rs));
            return reservation;
        }

        private void mapRenter(final ResultSet rs, final Reservation reservation)
            throws SQLException {
        ...
        private void mapVehicle(final ResultSet rs, final Reservation reservation)
            throws SQLException {
